...
(() => {
  ...
  var libh3 = (function(libh32) {
    ...
    var asm = (
      ...
      (function(global2, env, buffer2) {
        "almost asm";
        var a = new global2.Int8Array(buffer2), b = new global2.Int32Array(buffer2), c = new global2.Uint8Array(buffer2), d = new global2.Float32Array(buffer2), e = new global2.Float64Array(buffer2), f = env.o | 0, g = env.p | 0, p = global2.Math.floor, q = global2.Math.abs, r = global2.Math.sqrt, s = global2.Math.pow, t = global2.Math.cos, u = global2.Math.sin, v = global2.Math.tan, w = global2.Math.acos, x = global2.Math.asin, y = global2.Math.atan, z = global2.Math.atan2, A = global2.Math.ceil, B = global2.Math.imul, C = global2.Math.min, D = global2.Math.max, E = global2.Math.clz32, G = env.b, H = env.c, I = env.d, J = env.e, K = env.f, L = env.g, M = env.h, N = env.i, T = 28656;
  ...
  init_dist();
	  var MIGRATED_KEY = "bt_offline_storage_migrated";
	  async function prefersCapacitorStorage() {
	    return false;
	  }
  async function getOfflineStorageItem(key) {
    if (await prefersCapacitorStorage()) {
      const { Preferences: Preferences2 } = await Promise.resolve().then(() => (init_esm(), esm_exports));
      const { value } = await Preferences2.get({ key });
      return value;
    }
    try {
  ...
  async function setOfflineStorageItem(key, value) {
    if (await prefersCapacitorStorage()) {
      const { Preferences: Preferences2 } = await Promise.resolve().then(() => (init_esm(), esm_exports));
      await Preferences2.set({ key, value });
      return;
    }
    try {
  ...
  async function migrateLegacyOfflineStorage() {
  ...
  // lib/offline-buffer.ts
	  var PAINT_KEY = "bt_offline_paint";
	  var ERASE_KEY = "bt_offline_erase";
	  var GPS_PINGS_KEY = "bt_offline_gps_pings_v1";
	  var OFFLINE_TRANSFER_PREFIX = "bt-offline-transfer-v1:";
	  var initPromise = null;
  async function ensureOfflineBufferReady() {
    if (!initPromise) {
  ...
  async function getOfflinePaintQueue() {
    return readQueue(PAINT_KEY);
  }
	  async function getOfflineEraseQueue() {
	    return readQueue(ERASE_KEY);
	  }
	  async function readOfflineGpsPings() {
	    await ensureOfflineBufferReady();
	    try {
	      const raw = await getOfflineStorageItem(GPS_PINGS_KEY);
	      ...
	      for (const item of parsed) {
	        if (
	          ...
	          const cells = [...new Set(item.cells.filter((cell) => typeof cell === "string"))];
	  ...
	  async function writeOfflineGpsPings(pings) {
	    await setOfflineStorageItem(GPS_PINGS_KEY, JSON.stringify(pings));
	  }
	  function newGpsPingId() {
	    if (typeof crypto !== "undefined" && crypto.randomUUID) {
	      return `shell-gps-${crypto.randomUUID()}`;
	    }
	    return `shell-gps-${Date.now()}-${Math.random().toString(36).slice(2)}`;
	  }
	  async function appendOfflineGpsPing({ lat, lng, cells, recordedAtMs }) {
	    const uniqueCells = [...new Set(cells)];
	    if (uniqueCells.length === 0) return;
	    const pings = await readOfflineGpsPings();
	    pings.push({
	      id: newGpsPingId(),
	    ...
	    await writeOfflineGpsPings(pings);
	  }
	  async function getOfflineGpsStats() {
	    const pings = await readOfflineGpsPings();
  ...
  // lib/visit-batch-log.ts
  var BATCHES_KEY = "bt_visit_batches";
  var OPEN_BATCH_KEY = "bt_visit_open_batch";
  function newBatchId() {
    if (typeof crypto !== "undefined" && crypto.randomUUID) {
  ...
  async function readSealedBatches() {
    await ensureOfflineBufferReady();
    try {
  ...
  async function writeSealedBatches(batches) {
    await setOfflineStorageItem(BATCHES_KEY, JSON.stringify(batches));
  }
  async function readOpenBatch() {
    await ensureOfflineBufferReady();
    try {
  ...
  async function writeOpenBatch(batch) {
    await setOfflineStorageItem(OPEN_BATCH_KEY, JSON.stringify(batch));
  }
  async function appendPaintEvent(h3) {
    const open = await readOpenBatch();
    open.events.push({ op: "paint", h3, t: (/* @__PURE__ */ new Date()).toISOString() });
  ...
  // lib/sync-status-storage.ts
  var LAST_SYNC_KEY = "bt_last_sync_iso";
  async function getLastSyncIso() {
    return getOfflineStorageItem(LAST_SYNC_KEY);
  }

  // lib/shell-last-ping-storage.ts
  var KEY = "bt_shell_last_ping_json";
  async function setShellLastPing(p) {
    await setOfflineStorageItem(KEY, JSON.stringify(p));
  }
  async function getShellLastPing() {
    const raw = await getOfflineStorageItem(KEY);
    if (!raw) return null;
    try {
      const o = JSON.parse(raw);
      if (typeof o.lat === "number" && typeof o.lng === "number" && typeof o.atMs === "number" && typeof o.h3 === "string") {
        return o;
      }
    } catch {
    }
  ...
  function probeReachable(url, timeoutMs) {
    return new Promise((resolve) => {
      ...
      img.src = `${url}/favicon-32x32.png?probe=${Date.now()}`;
    });
  }
	  async function totalUnsynced() {
	    const [batchEvents, paints, erases, gpsStats] = await Promise.all([
	      countUnsyncedEvents(),
	      getOfflinePaintQueue(),
	      getOfflineEraseQueue(),
	      getOfflineGpsStats()
	    ]);
	    return batchEvents + paints.length + erases.length + gpsStats.visitEvents;
	  }
  function fmtTime(ms) {
    return new Date(ms).toLocaleTimeString(void 0, {
	  ...
	  async function prepareTransferToFullApp() {
	    await sealOpenBatch();
	    const [gpsPings, paints, erases, visitBatches, openBatch] = await Promise.all([
	      readOfflineGpsPings(),
	      getOfflinePaintQueue(),
	      getOfflineEraseQueue(),
	      readSealedBatches(),
  ...
  function scheduleSeal() {
    ...
    persistTimer = setTimeout(() => {
	  ...
	  async function refreshUi() {
	    const n = await totalUnsynced();
	    const gpsStats = await getOfflineGpsStats();
	    const syncIso = await getLastSyncIso();
	    el("unsynced").textContent = String(n);
	    el("cellCount").textContent = String(gpsStats.cellCount);
	    el("pingCount").textContent = String(gpsStats.pingCount);
	    el("eventCount").textContent = String(gpsStats.visitEvents);
	    el("lastSync").textContent = fmtIso(syncIso);
	    const ping = await getShellLastPing();
	    if (ping) {
	  ...
	  async function applyLocation(lat, lng, recordedAtMs = Date.now()) {
	    ...
	    if (prev.lat != null && prev.lng != null) {
	      const prevCell = snapToCell(prev.lat, prev.lng);
	      cellsToRecord = getCellsAlongLine(prevCell, newCell);
	    } else {
	      cellsToRecord = [newCell];
	    }
	    await appendOfflineGpsPing({ lat, lng, cells: cellsToRecord, recordedAtMs });
	    prev.lat = lat;
	    prev.lng = lng;
	    const atMs = recordedAtMs;
	    await setShellLastPing({ lat, lng, atMs, h3: newCell });
	    scheduleSeal();
	    await refreshUi();
	  }
	  async function startWatcher() {
	    ...
	    if (Capacitor.getPlatform() === "android") {
	      try {
	        const { LocalNotifications: LocalNotifications2 } = await Promise.resolve().then(() => (init_esm2(), esm_exports2));
        const perm = await LocalNotifications2.checkPermissions();
        if (perm.display !== "granted") await LocalNotifications2.requestPermissions();
      } catch {
      }
    }
    watcherId = await BackgroundGeolocation.addWatcher(
      {
        ...
        distanceFilter: NATIVE_DISTANCE_FILTER_M
	      ...
	      (location, error) => {
	        ...
	        void applyLocation(location.latitude, location.longitude, location.time ?? Date.now());
	      }
	    );
	    await refreshUi();
	  }
    ...
    window.setInterval(() => {
      ...
      void refreshUi();
	    ...
	    el("btnToggleTracking").addEventListener("click", async () => {
	      if (watcherId) {
	        await stopWatcher();
	        el("status").textContent = "Tracking paused. Saved visits stay on this phone.";
	      } else {
	        await startWatcher();
	        el("status").textContent = "Tracking locally. Saved visits will sync when online.";
	      }
	    });
	    await startWatcher();
  }
  void main().catch((e) => {
    el("status").textContent = `Error: ${e instanceof Error ? e.message : String(e)}`;
  });
})();
